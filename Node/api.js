...
---------------------------------------`);
	...
	CommandListQueued()
	{
		...
		Object.keys(this.queuedList).forEach((discordid, i) => {
			var rank = this.GetPlayerRank(discordid);
			var displayName = this.DiscordGetDisplayNameById(discordid)
			console.log(`(${rank.name}) @${displayName} [${this.onlineList[discordid].Username}] ${text(this.locale.general_priority)} ${this.queuedList[discordid].Priority}`);
		});
	}
